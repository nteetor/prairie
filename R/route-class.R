#' Route class
#' 
#' This class is a glorified container for a path and handler. Some handy
#' utility functions are included as private methods. As dull middleware begins
#' to take shape this class will evolve more.
#' 
#' @docType class
#' @keywords internal
...
route__ <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(method, path, handler, options = list()) {
    ...
    all = function(path, callback, ...) {
      ...
      private$append(private$.ALL, path, list(callback, ...))
      invisible(self)
    },
    get = function(path, callback, ...) {
      if (missing(path)) path <- private$path
      private$append('GET', path, list(callback, ...))
      invisible(self)
    },
    put = function(path, callback, ...) {
      if (missing(path)) path <- private$path
      private$append('PUT', path, list(callback, ...))
      invisible(self)
    },
    post = function(path, callback, ...) {
      if (missing(path)) path <- private$path
      private$append('POST', path, list(callback, ...))
      invisible(self)
    },
    route = function(path) {
    ...
    use = function(path = private$path, fn, ...) {
      private$append(private$.ALL, path, list(fn, ...))
      invisible(self)
    },
  ...
  private = list(
    ...
    append = function(method, path, callbacks) {
